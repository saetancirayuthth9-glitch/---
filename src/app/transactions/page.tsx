  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">ประวัติการเงินทั้งหมด</h1>
        ...
        {canManage && (
          <Link
            ...
            className="inline-flex items-center justify-center gap-2 bg-violet-600 hover:bg-violet-700 text-white px-5 py-2.5 rounded-2xl text-sm font-semibold transition-all shadow-md shadow-violet-200 active:scale-95 self-start sm:self-auto"
          >
      ...
      {/* Search and Filters */}
      <div className="bg-white p-4 sm:p-5 rounded-3xl border border-violet-100 shadow-sm flex flex-col sm:flex-row gap-3 sm:items-center justify-between">
        {/* Search Bar */}
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-violet-200 focus:border-violet-500 focus:bg-white transition-all"
          />
        </div>
      ...
      {/* Main List: Responsive Table for Desktop & Cards for Mobile */}
      <div className="bg-white rounded-3xl border border-violet-100 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center text-gray-400 space-y-2">
            ...
            <p className="text-base font-semibold text-gray-700">ไม่พบรายการที่ตรงกับการค้นหา</p>
          ...
          <>
            ...
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-gray-100 bg-violet-50/30 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                  ...
                  {filtered.map((tx) => (
                    <tr key={tx.id} className="hover:bg-violet-50/40 transition-colors">
                      ...
                      <td className="px-6 py-4 text-gray-600">
                        {tx.payer?.full_name ? (
                          <span className="font-medium text-gray-800">{tx.payer.full_name}</span>
                        ) : (
            ...
            <div className="md:hidden divide-y divide-gray-100">
              {filtered.map((tx) => (
                <div key={tx.id} className="p-4 space-y-2.5 hover:bg-violet-50/30 transition-colors">
                  ...
                  {/* Details line */}
                  <div className="flex flex-wrap items-center justify-between gap-2 pt-1 text-xs text-gray-500 border-t border-gray-50">
                    <div>
                      {tx.payer?.full_name ? (
                        <span>ผู้จ่าย: <strong className="text-gray-700">{tx.payer.full_name}</strong></span>
                      ) : (
